...
export async function generateStaticParams() {
  return getDetailsByPark("tds").map((d) => ({ slug: d.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const detail = getAttractionDetail("tds", slug);
  if (!detail) return {};
  const info = TDS_ATTRACTIONS.find((a) => a.id === detail.id);
  if (!info) return {};
  ...
  return {
    ...
    alternates: { canonical: `https://disneynow.tokyo/attractions/tds/${slug}` },
    openGraph: {
      title: `${info.nameJa} 待ち時間・攻略ガイド | TDLなう`,
      description: `${info.nameJa}の待ち時間・攻略情報。${info.descJa}`,
      url: `https://disneynow.tokyo/attractions/tds/${slug}`,
...
export default async function TDSAttractionDetailPage({ params }: Props) {
  const { slug } = await params;
  const detail = getAttractionDetail("tds", slug);
  if (!detail) notFound();
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <nav className="text-xs text-gray-400 mb-4">
          ...
          <Link href="/attractions/tds" className="hover:text-gray-600">シーガイド</Link>
          <span className="mx-1">/</span>
          <span className="text-gray-700">{info.nameJa}</span>
        </nav>

        <h1 className="text-xl font-bold text-gray-900 mb-1">{info.nameJa}</h1>
        <p className="text-sm text-gray-500 mb-4">{info.area}｜東京ディズニーシー</p>
        ...
        {/* リアルタイム待ち時間 */}
        <div className="mb-4">
          <AttractionWaitBadge parkId="tds" attractionId={info.id} />
        </div>
        ...
        <div className="mt-6 flex gap-3">
          <Link href="/attractions/tds" className="flex-1 text-center bg-gray-900 text-white rounded-xl py-3 text-sm font-semibold hover:bg-gray-700 transition-colors">
            シーのアトラクション一覧
          </Link>
      ...
      <Script id="jsonld-breadcrumb" type="application/ld+json" strategy="beforeInteractive" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 2, "name": "シーガイド", "item": "https://disneynow.tokyo/attractions/tds" },
          { "@type": "ListItem", "position": 3, "name": info.nameJa, "item": `https://disneynow.tokyo/attractions/tds/${detail.slug}` },
        ],
      })}} />
    </main>
  );
