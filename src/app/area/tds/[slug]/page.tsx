...
import { getAreaGuide, getAllAreaSlugs, TDS_AREA_GUIDES } from "@/lib/area-guide-data";
...
export async function generateStaticParams() {
  return getAllAreaSlugs("tds").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const area = getAreaGuide("tds", slug);
  if (!area) return {};
  return {
    title: `${area.name}完全攻略ガイド 2026 | アトラクション・フォトスポット・グルメ | TDLなう`,
    description: `東京ディズニーシー「${area.name}」の完全ガイド。${area.theme}をテーマにしたエリアのアトラクション情報・待ち時間傾向・フォトスポット・おすすめグルメ・回り方のコツを徹底解説。`,
    alternates: { canonical: `https://disneynow.tokyo/area/tds/${slug}` },
  };
}

const TDSMAP: { slug: string; name: string }[] = TDS_AREA_GUIDES.map((g) => ({ slug: g.slug, name: g.name }));
...
export default async function TdsAreaPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const area = getAreaGuide("tds", slug);
  if (!area) notFound();
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <nav className="text-xs text-gray-400 mb-4">
          ...
          <Link href="/attractions/tds" className="hover:text-gray-600">TDSガイド</Link>
          <span className="mx-1">/</span>
        ...
        <div className={`relative rounded-2xl overflow-hidden mb-6 bg-gradient-to-br ${area.heroGradient}`} style={{ height: 220 }}>
          ...
          <div className="absolute bottom-0 left-0 p-5">
            <p className="text-xs text-white/70 mb-1">東京ディズニーシー</p>
        ...
        <section className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm mb-5">
          ...
          <div className="mt-4 flex items-center gap-2 bg-teal-50 rounded-xl p-3">
            <span className="text-lg">⏰</span>
            <div>
              <p className="text-xs font-bold text-teal-800">おすすめ訪問時間帯</p>
              <p className="text-xs text-teal-700 mt-0.5">{area.bestTime} — {area.bestTimeDetail}</p>
            </div>
          </div>
        </section>
            ...
            {area.food.map((f, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <span className="text-xs bg-teal-100 text-teal-700 px-2 py-0.5 rounded-full shrink-0">{f.type}</span>
                </div>
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {TDSMAP.filter((a) => a.slug !== slug).map((a) => (
              <Link
                key={a.slug}
                href={`/area/tds/${a.slug}`}
        ...
        <div className="flex flex-col gap-2">
          <Link href="/" className="flex items-center justify-between bg-teal-600 rounded-2xl p-4 shadow-sm hover:bg-teal-700 transition-colors">
          ...
          <Link href="/attractions/tds" className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl p-4 shadow-sm hover:border-gray-400 transition-colors">
            <div>
              <p className="text-sm font-bold text-gray-900">TDSアトラクションガイド一覧</p>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-4">
        <AmazonBanner keyword="ディズニーシー グッズ エリア" label="Amazonでディズニーシーグッズを探す" />
      </div>
      ...
      <Script id={`area-tds-${slug}-jsonld`} type="application/ld+json" dangerouslySetInnerHTML={{
        __html: JSON.stringify({
          "@context": "https://schema.org",
          "@type": "Article",
          "headline": `${area.name}完全攻略ガイド 2026｜東京ディズニーシー`,
          "description": `東京ディズニーシー「${area.name}」の攻略ガイド。アトラクション情報・フォトスポット・グルメ・回り方のコツを徹底解説。`,
