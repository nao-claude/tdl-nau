...
export function WaitTimeCardEn({ attraction, isFavorite, onToggleFavorite }: Props) {
  const { name, is_open, wait_time } = attraction;
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        {imageUrl ? (
          <div className={`w-14 h-14 rounded-xl overflow-hidden shrink-0 ${!is_open ? "opacity-40 grayscale" : ""}`}>
            <Image src={imageUrl} alt={name} width={56} height={56} className="w-full h-full object-cover" unoptimized />
          </div>
        ) : (
          ...
          {passes.length > 0 && (
            <div className="flex gap-1 mt-1">
              ...
              {passes.includes("special") && (
                <span className="text-xs font-bold px-1.5 py-0.5 rounded bg-amber-500 text-white leading-none">Priority Pass</span>
              )}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        {is_open ? (
          wait_time > 0 ? (
            ...
            <span className="text-xs text-green-600 font-medium">No wait</span>
          )
        ) : (
          <span className="text-xs text-gray-400 flex items-center gap-0.5"><XCircle className="w-3 h-3" />Closed</span>
        )}
      </div>
    </div>
  );
